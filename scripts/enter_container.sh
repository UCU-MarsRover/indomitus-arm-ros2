...
Commands:
  ...
  arm         Connects to the Jetson Nano, starts the container, and opens a terminal.

Use '$0 local --help' or '$0 arm --help' for command-specific options.
EOF
}
...
run_arm() {
  ...
  local REMOTE_DIR="/home/ros/Indomitus/indomitus-arm-ros2/"
  local CONTAINER_NAME="arm_prod"
...
EOF
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      -h|--help)    show_arm_help; exit 0;;
      *)            error "Unknown option: $1\nRun '$0 arm --help' for usage." ;;
    esac
  done
  ...
  step "Running Pre-Flight Checks (Arm)..."
...
case "$COMMAND" in
  ...
  -h|--help)
    ...
    ;;
  *)
    error "Unknown command: '$COMMAND'. Must be 'local' or 'arm'.\nRun '$0 --help' for usage."
    ;;
esac